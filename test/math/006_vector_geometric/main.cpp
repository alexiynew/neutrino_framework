...
int main()
{
    ...
    // vector geometric functions
    {
        ...
        ASSERT_MSG(almost_equal(distance(Vector3F(3.0f, 3.0f, 5.0f), Vector3F(2.0f, 1.0f, 2.0f)), length(v3f)) &&
                   almost_equal(distance(Vector3F(2.0f, 1.0f, 2.0f), Vector3F(3.0f, 3.0f, 5.0f)), length(v3f)),
                   "distance failed");
        ...
        ASSERT_MSG(almost_equal(refraction(v3f, normalize(v3f), 1.0f), reflection(v3f, normalize(v3f))),
                   "refraction "
                   "failed");
    }
