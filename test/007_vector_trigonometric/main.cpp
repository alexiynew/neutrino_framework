...
int main()
{
    ...
    // vector trigonometric functions
    {
        ...
        ASSERT_MSG(
        almostEqual(asin(sin(radians(v4d))), Vector4D(asin(sin(PI)), asin(sin(TAU)), PI / 2, PI / 4), 1), "asin failed");
        ...
        ASSERT_MSG(almostEqual(atan(sin(radians(v3d)), cos(radians(v3d))), Vector3D(PI / 4, PI / 3, PI)), "atan from sin "
                                                                                                        "failed");
