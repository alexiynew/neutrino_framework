...
{
    static_assert(utils::is_floating_point_or_integer<T>::value && utils::is_floating_point_or_integer<U>::value, "Expected floating-point or integer type.");
...
template <typename T, template <U32, typename> class TVec, typename TResult = decltype(std::frexp(std::declval<const T&>(), std::declval<I32*>()))>
...
{
    return almostEqual(lhs.x, rhs.x, ulp) && almostEqual(lhs.y, rhs.y, ulp) && almostEqual(lhs.z, rhs.z, ulp) && almostEqual(lhs.w, rhs.w, ulp);
}
...
{
    return almostEqual(m[0], m1[0], ulp) && almostEqual(m[1], m1[1], ulp) && almostEqual(m[2], m1[2], ulp) && almostEqual(m[3], m1[3], ulp);
}
